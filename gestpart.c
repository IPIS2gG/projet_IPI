...
/* alignement détecte les alignements de 5 autour de la case  */
...
int alignement5possible(partie *p, int x, int y){
  ...
  for (i=-4 ; i<1 ; i++)
      ...
      for (j=i ; j<i+5 ; j++)
	{
      ...
      if (compteur == 5)
	{// un alignement de 5 est possible
	  //printf("ligne\n");
	  return 0; 
	}
    }
...
int play(partie *p, int x, int y, int joueur)
{
  ...
  // case vide
  else 
    {
      ...
      if (alignement5(p, x, y, joueur) == 0)
	{
	  printf("joueur %d : alignement de 5 : (%d, %d)\n", joueur, x, y);
	  // mise à jour de la map faite dans fonction alignement5

	  // mise a jour du score du joueur
	  p->score[joueur-1]++;	    
	}

      if (is_fin_du_jeu(p) == 0)
	{
